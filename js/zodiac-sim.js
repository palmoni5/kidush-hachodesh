...
(function () {
  ...
  function drawExternal(ctx, W, H, date, st) {
    ...
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    // שם התקופה נכתב אצל המזל שהשמש נראית בו אז — תקופת ניסן בראש טלה,
    // תמוז בסרטן, תשרי במאזניים וטבת בגדי — שהמזל נקבע במקום השמש הנראית
    // ולא במקום הארץ. הנקודה שמנגד, על המסלול, היא מקום הארץ באותה תקופה:
    // היא עומדת תמיד במזל שכנגד, וקו הראייה שדרך השמש הוא המחבר ביניהם.
    // (קודם נכתב השם על הנקודה שבמסלול, ונמצא צמוד למזל שכנגד — ואף נחבא
    // תחת כדור הארץ בהגיעו לשם.)
    for (const [sl, name] of [[0, 'ניסן'], [90, 'תמוז'], [180, 'תשרי'], [270, 'טבת']]) {
      const p = px(sl + 180, orbR), q = px(sl + 9, innerR + 9);
      ctx.fillStyle = cv('--ill-muted') || '#999';
      ctx.beginPath(); ctx.arc(p.x, p.y, 2, 0, 2 * PI); ctx.fill();
      // הכיתוב יושב בשפתה הפנימית של הרצועה, בתחילת המזל עצמו (9° לתוכו) —
      // פנוי מתווית המזל שבאמצע הרצועה, ומן התוויות שבתוך הגלגל ("כאן נראית
      // השמש" וחברותיה), שתיבתן נמתחת בדיוק על קו המחיצה בהגיע השמש לתקופה
      ctx.fillStyle = cv('--ill-text') || '#e0e0e0';
