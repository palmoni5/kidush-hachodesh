...
(function () {
  ...
  //   grimt — הלבוש כפירוש הגרי״מ טוקצינסקי: שש שעות אחר החצות האמיתי;
  ...
  const SHITOT = [
    ...
    { key: 'grimt', label: 'הגרי״מ טוקצינסקי — חצות אמיתי', src: 'הלבוש סי׳ תכ״ח: היממה משש שעות אחר חצות; לפי החצות האמיתי של כל יום' },
  ...
  function renderSummary(t) {
    ...
    // שנה שקודם תקנת הלל הנשיא (ד׳קי״ט) — אז קידשו על פי הראייה, והטבלה
    // אינה אלא החשבון הקבוע כשהוא נמשך לאחור (ראו "מקורות והערות")
    if (t.year && t.year < HILLEL_YEAR)
      msgs.push(T('שנה זו קודמת לתקנת הלל הנשיא (שנת ד׳קי״ט) — בזמן ההוא קידשו את החודשים על פי הראייה ועיברו את השנים בבית דין, והלוח שכאן אינו אלא החשבון הקבוע כשהוא נמשך לאחור.'));
    w.innerHTML = msgs.map(m => `<div>⚠ ${m}</div>`).join('');
    w.style.display = msgs.length ? '' : 'none';
  }

  const HILLEL_YEAR = 4119;   // קידוש החודשים על פי החשבון לדורות — בשם רב האי גאון
