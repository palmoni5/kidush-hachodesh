...
//   125.24°E  — קו התאריך שיטת החזון איש (90° מזרחית לירושלים)
//   144.76°W  — קו התאריך שיטת הגרי"מ טיקוצינסקי (180° מירושלים)
...
(function () {
  ...
  // נקודת הציון של ירושלים — הר הבית, 31°46′41″צ / 35°14′07″מז, שהם
  // 31.78° / 35.24° בשבר עשרוני. שימו לב: מעלות ודקות אינן שבר עשרוני —
  // 35°14′ הן 35.23° ולא 35.14°, וכך מוצג הדבר בגוגל־ארט כברירת מחדל.
  // אותה נקודה עצמה משמשת בכל הלשוניות.
  const JLEM_LON = 35.24, JLEM_LAT = 31.78;
  const CI_LON   = JLEM_LON + 90;            // 125.24°    — חזון איש
  const GRIT_LON = JLEM_LON - 180;           // -144.76°   — הגרי"מ טיקוצינסקי
  ...
  // שטח הספק של הגרי"מ טיקוצינסקי: מערב אלסקה — מקו התאריך שלו (144.77°W)
  ...
  const sim = {
    ...
    draw() {
      ...
      if (this.show.grit) this.label(ctx, T('גרי״מ טיקוצינסקי'), GRIT_LON, cx, cy, R, COL.grit, 12);
