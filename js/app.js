  ...
  function enableTabScroll(bar) {
    bar.addEventListener('wheel', e => {
      ...
      if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) {
        // מחווה אופקית (שתי אצבעות במשטח המגע): סימן deltaX תלוי-מארח —
        // ב-webview של אוצריא הוא מגיע הפוך על מסמך RTL (תנועת אצבעות ימינה
        // גררה את הפס שמאלה), ואילו ב"כלי קודש" ובדפדפן הוא תקני; אותו קוד
        // עצמו התנהג הפוך בשני המארחים (משוב משתמש בשניהם, אלול תשפ"ו).
        // לכן ההיפוך מוחל רק כשגשר Otzaria הוזרק על ידי התוכנה — זיהוי
        // אוצריא (ראו theme.js) — וכך התוכן עוקב אחר האצבעות בכל המארחים.
        bar.scrollLeft += (window.__hasOtzariaBridge ? -1 : 1) * e.deltaX;
      } else {
