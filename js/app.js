...
(function () {
  ...
  // ── מצב קירוב גלוי ──────────────────────────────────────────────────
  // כשמנוע האסטרונומיה (vendor/astronomy.browser.min.js) אינו נטען, האיורים
  // נופלים בשקט לחישובים הסכמטיים (שנת שמואל, חודש ממוצע, נוסחת נטייה
  // מקורבת — סטייה של עד ~1.6°). הנפילה מסומנת למשתמש, שלא יקרא ערכים
  // סכמטיים כערכים אסטרונומיים מדויקים. הטקסט מיתרגם עם שאר ה-DOM.
  if (typeof window.Astronomy === 'undefined') {
    const warn = document.createElement('div');
    warn.className = 'luach-warn';
    warn.style.margin = '8px 10px 0';
    warn.textContent = 'מנוע החישוב האסטרונומי לא נטען — האיורים מוצגים במצב קירוב סכמטי (שנת שמואל וחודש ממוצע), לא בערכים אסטרונומיים מדויקים.';
